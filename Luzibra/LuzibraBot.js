...
const ts3 = new TeamSpeak({
  host: "31.97.30.1",        // IP do servidor TS3
  queryport: 10101,             // Porta do query
  serverport: 9992,             // Porta do servidor TS3
...
const SPECIAL_GROUPS_FILE = './special_group.json';

// Inicializa os grupos especiais com valores padrão
let specialGroups = [
  'Editor',
  'Server Admin',
  'Guest',
...
// Função para carregar grupos especiais do arquivo
function loadSpecialGroups() {
  try {
    if (fsr.existsSync(SPECIAL_GROUPS_FILE)) {
      specialGroups = JSON.parse(fsr.readFileSync(SPECIAL_GROUPS_FILE, 'utf8'));
      console.log('Grupos especiais carregados:', specialGroups.length);
    } else {
      // Cria o arquivo se não existir com os grupos padrão
      saveSpecialGroups();
      console.log('Arquivo de grupos especiais criado com valores padrão.');
    }
  } catch (error) {
...
// Função para salvar grupos especiais no arquivo
function saveSpecialGroups() {
  try {
    fsr.writeFileSync(SPECIAL_GROUPS_FILE, JSON.stringify(specialGroups, null, 2), 'utf8');
    console.log('Grupos especiais salvos com sucesso.');
  } catch (error) {
    console.error('Erro ao salvar grupos especiais:', error);
  }
}

// Carregar grupos especiais ao iniciar
loadSpecialGroups();

// Função para adicionar um grupo especial
async function addSpecialGroup(clientId, groupName) {
  try {
    // Verificar permissões do usuário
    const client = await getFullClientInfo(clientId);
    if (!client) {
      console.error(`Não foi possível obter informações do cliente ${clientId}`);
      return;
    }

    const hasPermission = await hasEditorPermission(client);
    if (!hasPermission) {
      await sendSafeTextMessage(clientId, "Você não tem permissão para adicionar grupos especiais.");
      return;
    }

    // Verificar se o grupo já existe
    if (specialGroups.includes(groupName)) {
      await sendSafeTextMessage(clientId, `O grupo "${groupName}" já existe na lista de grupos especiais.`);
      return;
    }
    ...
    if (!groupId) {
      await sendSafeTextMessage(clientId, `O grupo "${groupName}" não existe no servidor TeamSpeak.`);
      return;
    }

    // Adicionar o grupo à lista
    specialGroups.push(groupName);
    saveSpecialGroups();

    await sendSafeTextMessage(clientId, `Grupo "${groupName}" adicionado com sucesso à lista de grupos especiais.`);
    console.log(`Grupo "${groupName}" adicionado à lista de grupos especiais por ${client.clientNickname}`);
  } catch (error) {
    console.error(`Erro ao adicionar grupo especial:`, error);
    await sendSafeTextMessage(clientId, "Ocorreu um erro ao adicionar o grupo especial.");
  }
}

// Função para remover um grupo especial
async function removeSpecialGroup(clientId, groupName) {
  try {
    // Verificar permissões do usuário
    const client = await getFullClientInfo(clientId);
    if (!client) {
    ...
    const hasPermission = await hasEditorPermission(client);
    if (!hasPermission) {
      await sendSafeTextMessage(clientId, "Você não tem permissão para remover grupos especiais.");
      return;
    }

    // Verificar se o grupo existe na lista
    const index = specialGroups.indexOf(groupName);
    if (index === -1) {
      await sendSafeTextMessage(clientId, `O grupo "${groupName}" não existe na lista de grupos especiais.`);
      return;
    }
    ...
    await sendSafeTextMessage(clientId, `Grupo "${groupName}" removido com sucesso da lista de grupos especiais.`);
    console.log(`Grupo "${groupName}" removido da lista de grupos especiais por ${client.clientNickname}`);
  } catch (error) {
    console.error(`Erro ao remover grupo especial:`, error);
    await sendSafeTextMessage(clientId, "Ocorreu um erro ao remover o grupo especial.");
  }
}

// Função para visualizar os grupos especiais
async function viewSpecialGroups(clientId) {
  try {
    // Verificar permissões do usuário
    const client = await getFullClientInfo(clientId);
    if (!client) {
    ...
    const hasPermission = await hasEditorPermission(client);
    if (!hasPermission) {
      await sendSafeTextMessage(clientId, "Você não tem permissão para visualizar a lista de grupos especiais.");
      return;
    }

    // Formatar a lista de grupos
    const groupsList = specialGroups.map((group, index) => `${index + 1}. ${group}`).join('\n');
    const message = `[b]Lista de Grupos Especiais:[/b]\n\n${groupsList}`;

    await sendSafeTextMessage(clientId, message);
    console.log(`Lista de grupos especiais visualizada por ${client.clientNickname}`);
  } catch (error) {
    console.error(`Erro ao visualizar grupos especiais:`, error);
    await sendSafeTextMessage(clientId, "Ocorreu um erro ao visualizar a lista de grupos especiais.");
  }
}
...
// Arquivo para armazenar usuários com o grupo Claimed
const CLAIMED_USERS_FILE = './claimed_users.json';
...
// Inicializa o registro de usuários com o grupo Claimed
let claimedUsers = {};
// Função para verificar e corrigir o formato do arquivo claimed_users.json
function fixClaimedUsersFile() {
  try {
    console.log('Verificando formato do arquivo claimed_users.json...');
    
    // Verificar se o arquivo existe
    if (fsr.existsSync(CLAIMED_USERS_FILE)) {
      // Ler o conteúdo atual
      const content = fsr.readFileSync(CLAIMED_USERS_FILE, 'utf8');
      let data;
      
      try {
        data = JSON.parse(content);
        
        // Verificar se é um array vazio ou não é um objeto
        if (Array.isArray(data) || typeof data !== 'object' || data === null) {
          console.log('Formato incorreto detectado no claimed_users.json. Corrigindo para objeto vazio...');
          // Substituir por um objeto vazio
          fsr.writeFileSync(CLAIMED_USERS_FILE, JSON.stringify({}, null, 2), 'utf8');
          claimedUsers = {};
          console.log('Arquivo claimed_users.json corrigido para objeto vazio.');
        } else {
          console.log('Formato do arquivo claimed_users.json está correto.');
          claimedUsers = data;
        }
      } catch (parseError) {
        console.error('Erro ao analisar claimed_users.json:', parseError);
        // Se não for possível analisar, recriar como objeto vazio
        fsr.writeFileSync(CLAIMED_USERS_FILE, JSON.stringify({}, null, 2), 'utf8');
        claimedUsers = {};
        console.log('Arquivo claimed_users.json recriado como objeto vazio devido a erro de análise.');
      }
    } else {
      // Criar o arquivo se não existir
      fsr.writeFileSync(CLAIMED_USERS_FILE, JSON.stringify({}, null, 2), 'utf8');
      claimedUsers = {};
      console.log('Arquivo claimed_users.json criado como objeto vazio.');
    }
  } catch (error) {
    console.error('Erro ao verificar/corrigir claimed_users.json:', error);
    // Garantir que claimedUsers seja um objeto vazio em caso de erro
    claimedUsers = {};
  }
}

// Chamar a função para verificar e corrigir o formato do arquivo
fixClaimedUsersFile();
...
// Função para salvar usuários do grupo Claimed
function saveClaimedUsers() {
  try {
      // Garantir que claimedUsers seja um objeto antes de salvar
      if (!claimedUsers || typeof claimedUsers !== 'object' || Array.isArray(claimedUsers)) {
          console.error('Erro: claimedUsers não é um objeto válido. Inicializando como objeto vazio.');
          claimedUsers = {};
      }
      
      fsr.writeFileSync(CLAIMED_USERS_FILE, JSON.stringify(claimedUsers, null, 2), 'utf8');
      console.log('Usuários do grupo Claimed salvos com sucesso.');
  } catch (error) {
      console.error('Erro ao salvar usuários do grupo Claimed:', error);
  }
}
...
function getGuildAliada() {
  try {
      ...
      return decodeIfEncoded(setGuild.guild || 'New Coorporative'); // Garante que o nome não esteja codificado
  } catch (error) {
      console.error('Erro ao carregar a guilda aliada:', error);
      return 'New Coorporative'; // Valor padrão se houver erro ou o arquivo estiver vazio
  }
}
...
async function sendAllRegistrationInstructions(clid, code) {
  try {
      ...
      `[color=red][b]!register SeuPersonagem[/b][/color]\n\n` +
...
try {
    ...
    // Verificar se o personagem pertence à guilda
    await sendSafeTextMessage(clid, `Verificando se o personagem ${character.name} pertence à guilda ${guildAliada}...`);
    const guildMember = await checkTibiaCharacterInGuild(character.name);
    
    if (!guildMember) {
        console.log(`Personagem ${character.name} não pertence à guilda ${guildAliada}`);
        await sendSafeTextMessage(clid, 
            `O personagem ${character.name} não pertence à guilda ${guildAliada}. Apenas membros da guilda podem se registrar.`);
        return;
    }
    
    console.log(`Personagem ${characterName} verificado com sucesso e pertence à guilda!`);
    ...
    const confirmationMessage = `[b][color=green]Personagem encontrado![/color][/b]\n\n` +
        ...
        `[b]Vocação:[/b] ${character.vocation}\n` +
    ...
    registeredUsers[clientUniqueIdentifier] = {
        ...
        guildRank: guildMember.rank || null // Usar o rank obtido da verificação da guilda
    };
    ...
    await sendSafeTextMessage(clid, 
        `[b][color=green]Registro concluído com sucesso![/color][/b]\n\n` +
        `Seu personagem ${character.name} foi registrado e seus grupos foram atualizados de acordo com seu level, vocação e rank na guilda.`);
...
try {
    ...
    // Verificar se o personagem pertence à guilda
    await sendSafeTextMessage(editorClid, `Verificando se o personagem ${character.name} pertence à guilda ${guildAliada}...`);
    const guildMember = await checkTibiaCharacterInGuild(character.name);
    
    if (!guildMember) {
        console.log(`Personagem ${character.name} não pertence à guilda ${guildAliada}`);
        await sendSafeTextMessage(editorClid, 
            `O personagem ${character.name} não pertence à guilda ${guildAliada}. Apenas membros da guilda podem ser registrados.`);
        return;
    }
    ...
    const confirmationMessage = `[b][color=green]Personagem encontrado![/color][/b]\n\n` +
        ...
        `[b]Guilda:[/b] ${guildAliada}\n` +
        `[b]Rank:[/b] ${guildMember.rank || "N/A"}\n\n` +
        `[color=yellow]Buscando usuário para registrar...[/color]`;
...
async function getCurrentManagedGroups(clientGroups) {
  ...
  for (const group of allGroups) {
      ...
      const isSpecialGroup = specialGroups.includes(group.name); // Aqui está a correção: specialGroups em vez de SPECIAL_GROUPS
...
// Função para verificar se um usuário está no grupo Claimed e gerenciar seu status
async function checkClaimedUsers() {
  try {
      console.log('Verificando usuários do grupo Claimed...');
      const clients = await ts3.clientList();
      const claimedGroupId = await getGroupIdByName('Claimed');
      
      if (!claimedGroupId) {
          console.error('Grupo Claimed não encontrado');
          return;
      }

      // Verificar todos os clientes online
      for (const client of clients) {
          if (!client || !client.clid) continue;
          ...
          const { clientUniqueIdentifier, clientServergroups, clientDatabaseId, clientNickname } = clientInfo;
          
          // Converter clientServergroups para array de números
          const groups = typeof clientServergroups === 'string' 
              ? clientServergroups.split(',').map(Number) 
              : Array.isArray(clientServergroups) 
                  ? clientServergroups.map(Number) 
                  : [];
          
          // Verificar se o usuário está no grupo Claimed
          const isInClaimedGroup = groups.includes(Number(claimedGroupId));
          
          if (isInClaimedGroup) {
              // Se o usuário está no grupo Claimed mas não está no nosso rastreamento, adicione-o
              if (!claimedUsers[clientUniqueIdentifier]) {
                  console.log(`Adicionando ${clientNickname} ao rastreamento de usuários do grupo Claimed`);
                  claimedUsers[clientUniqueIdentifier] = {
                      timestamp: Date.now(),
      ...
      // Verificar usuários do grupo Claimed expirados (24 horas = 86400000 ms)
      const now = Date.now();
      const expiredUsers = [];
      
      for (const [uniqueId, userData] of Object.entries(claimedUsers)) {
          const timeElapsed = now - userData.timestamp;
          
          if (timeElapsed >= 86400000) { // 24 horas
              console.log(`Usuário ${userData.clientNickname} excedeu 24 horas no grupo Claimed`);
              expiredUsers.push({ uniqueId, ...userData });
          }
      }
      
      // Remover usuários expirados do grupo Claimed
      for (const user of expiredUsers) {
          try {
              await ts3.clientDelServerGroup(user.clientDatabaseId, claimedGroupId);
              console.log(`Removido ${user.clientNickname} do grupo Claimed (24h expiradas)`);
              
              // Remover do rastreamento
              delete claimedUsers[user.uniqueId];
              
              // Notificar o usuário se estiver online
              const onlineClient = clients.find(c => 
                  c.client_database_id === user.clientDatabaseId.toString()
              );
              
              if (onlineClient) {
                  await sendSafeTextMessage(onlineClient.clid, 
                      "Você foi removido do grupo 'Claimed' pois o período de 24 horas expirou.");
              }
          } catch (error) {
              console.error(`Erro ao remover ${user.clientNickname} do grupo Claimed:`, error);
          }
      }
      ...
      console.error('Erro ao verificar usuários do grupo Claimed:', error);
  }
}
...
async function processCommand(message, clid) {
  ...
  // NOVOS COMANDOS PARA GERENCIAR GRUPOS ESPECIAIS
  
  // Comando para adicionar grupo especial
  if (normalizedMessage.startsWith("!addgroup ")) {
      console.log(`Comando !addgroup recebido de ${clid}`);
      
      const groupName = message.substring("!addgroup ".length).trim();
      if (!groupName) {
          await sendSafeTextMessage(clid, "Por favor, forneça o nome do grupo. Exemplo: !addgroup NomeDoGrupo");
          return;
      }
      
      await addSpecialGroup(clid, groupName);
      return;
  }
  ...
  // Comando para remover grupo especial
  if (normalizedMessage.startsWith("!removegroup ")) {
      console.log(`Comando !removegroup recebido de ${clid}`);
      
      const groupName = message.substring("!removegroup ".length).trim();
      if (!groupName) {
          await sendSafeTextMessage(clid, "Por favor, forneça o nome do grupo. Exemplo: !removegroup NomeDoGrupo");
          return;
      }
      
      await removeSpecialGroup(clid, groupName);
      return;
  }
  
  // Comando para visualizar grupos especiais
  if (normalizedMessage === "!viewgroup") {
      console.log(`Comando !viewgroup recebido de ${clid}`);
      await viewSpecialGroups(clid);
      return;
  }
}
...
ts3.on('ready', async () => {
  ...
  setInterval(async () => {
      try {
          ...
          // Verificar usuários do grupo Claimed
          await checkClaimedUsers();
...
const adminGroupID = 1108;
const editor = 1063;
const respconfiga = 1126;
const respconfigb = 1110;
const respconfigc = 1131;
const serveradminGroupID = 3;
const mpoke =  1111;
const mmove = 1112;
const botadm = 1110;
const respblockGroupID = 1124;
const convidado = 1121;
const SemRegistro = 1065;
const planilhado = 1130;

//////// canais /////////
const canalAFK = 132;
const canalResp = 15;
const canalGuildAliada = 5;
const canalEnemy = 10;
const canalHuntedIndividual = 8;
...
async function getFullClientInfo(clid) {
  try {
      ...
      const clientServerGroups = info.servergroups || []; // IDs dos grupos do cliente
      ...
      // Verificar se o cliente tem o grupo "No Move"
      let hasNoMoveGroup = false;
      
      // Obter os nomes dos grupos do cliente
      for (const groupId of clientServerGroups) {
          try {
              const serverGroupInfo = await ts3.serverGroupList({ sgid: groupId });
              if (serverGroupInfo && serverGroupInfo.length > 0) {
                  const groupName = serverGroupInfo[0].name;
                  if (groupName === "No Move") {
                      hasNoMoveGroup = true;
                      break;
                  }
              }
          } catch (groupError) {
              console.error(`Erro ao obter informações do grupo ${groupId}:`, groupError);
          }
      }
      
      if (hasNoMoveGroup) {
          console.log(`${clientNickname} tem o grupo "No Move" e não será movido para o AFK.`);
          return info;
      }
...
async function handleAfkCommand(msg, invoker) {
    ...
    try {
        ...
        if (![adminGroupID, serveradminGroupID, botadm, editor].some(group => clientServerGroups.includes(group.toString()))) {
            ts3.sendTextMessage(invokerID, 1, "Você não tem permissão para usar esse comando.");
            return;
        }
        ...
        if (args.length === 1) {
            ...
            if (isNaN(newAfkTime) || newAfkTime <= 0) {
                ts3.sendTextMessage(invokerID, 1, "Por favor, diga um valor válido de minutos para o tempo AFK.");
            } else {
                afkTime = newAfkTime;
...
async function sendLevelMessage(playerName, oldLevel, newLevel, action) {
  ...
  try {
    ...
    await ts3.sendTextMessage(1, `[B][color=${color}]${message}[/color][/B]`)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if (message.startsWith("!scan")) {
        try {
            ...
            const clientServerGroups = clientInfo.servergroups || [];
            if (![editor, botadm].some(group => clientServerGroups.includes(group.toString()))) {
...
if (message.startsWith("!shared")) {
  ...
  if (message.startsWith("!mp")) {
      try {
          ...
          if ([adminGroupID, serveradminGroupID, editor, botadm, mpoke].some(group => clientServerGroups.includes(group.toString()))) {
...
async function updateRespawnChannel() {
  try {
    ...
    let description = "[size=+3]RESPAWN LIST[/size]\n[i]${formattedDate}[/i]\n[url=https://luzibra.jowbot.com.br][size=+2][b]CLIQUE AQUI PARA ACOMPANHAR PELO NAVEGADOR OU CELULAR[/b][/size][/url]\n\n";
    ...
    // Coletar todos os respawns com seus nomes para ordenação
    ...
    // Ordenar os respawns por nome em ordem alfabética
    respawnsToProcess.sort((a, b) => a.respawnName.localeCompare(b.respawnName))
    
    console.log(`[UPDATE] Processando ${respawnsToProcess.length} respawns ordenados alfabeticamente`)

    // Processar os respawns já ordenados
    for (const { respawnNumber, respawnName, respawn } of respawnsToProcess) {
      ...
      try {
        ...
        if (respawn.waitingForAccept) {
          ...
          if (respawn.pausado) {
            ...
            // Se temos o tempo restante salvo, calculamos o tempo decorrido a partir dele
            if (respawn.tempoRestantePausa !== null && respawn.tempoRestantePausa !== undefined) {
              // Converter o tempo restante de milissegundos para minutos
              const tempoRestanteMinutos = Math.ceil(respawn.tempoRestantePausa / 60000)
              
              // Calcular o tempo decorrido (total - restante)
              const totalMinutos = respawn.time
              const decorrido = totalMinutos - tempoRestanteMinutos
              
              // Garantir que o tempo decorrido não seja negativo
              const decorridoAjustado = Math.max(0, decorrido)
              const decorridoHoras = Math.floor(decorridoAjustado / 60)
              const decorridoMinutos = decorridoAjustado % 60
              
              elapsedTime = `${decorridoHoras.toString().padStart(2, "0")}h${decorridoMinutos.toString().padStart(2, "0")}m`
            } 
            // Se temos a data de pausa, usamos ela para calcular o tempo decorrido até o momento da pausa
            else if (respawn.pauseDate) {
              const startDate = new Date(respawn.startTime)
              const pauseDate = new Date(respawn.pauseDate)
              const elapsedMs = pauseDate - startDate
            ...
            // Fallback: usar o tempo atual (não ideal, mas evita erros)
            else {
              const startDate = new Date(respawn.startTime)
              const now = new Date()
            ...
            // Tempo formatado com indicação de pausado
            tempoText = `[b][color=#1D8F24]${elapsedTime}/${totalTime} [color=red][PAUSADO][/color][/color][/b]`
          } else {
  ...
  try {
    ...
    for (const respawnKey in filaRespawns) {
      ...
      if (!respawn || !respawn.current) {
        console.log(`Respawn ${respawnKey} não tem estrutura válida, pulando...`)
        continue // Não remover, apenas pular
      }

      // Verificar se o respawn está pausado individualmente
      if (respawn.pausado === true) {
        console.log(`Respawn ${respawnKey} está pausado individualmente, pulando processamento de tempo`)
        continue
      }

      // Verificar se está aguardando aceitação
      if (respawn.waitingForAccept === true) {
        ...
        if (respawn.acceptanceTime <= 0) {
          ...
          if (respawn.queue && respawn.queue.length > 0) {
            ...
            // Verificar se o cliente tem um tempo personalizado
            if (nextClient.customTime !== null && nextClient.customTime !== undefined) {
              // Usar o tempo personalizado que foi armazenado
              respawn.time = nextClient.customTime
              console.log(`Usando tempo personalizado para o próximo cliente: ${respawn.time} minutos`)
            } else {
              // Se não tiver tempo personalizado, calcular com base no grupo
              try {
                ...
                try {
                  ...
                  const matchingClient = allClients.find(
                    (c) => c.uniqueIdentifier === nextClient.clientUniqueIdentifier,
                  )
                ...
                if (nextClientInfo) {
                  ...
                  console.log(`Tempo baseado no grupo definido para o próximo cliente: ${respawn.time} minutos`)

                  // Aplicar o limite máximo absoluto de 3:30 horas (210 minutos)
                  const MAX_RESPAWN_TIME = 210 // 3 horas e 30 minutos
                  if (respawn.time > MAX_RESPAWN_TIME) {
                    console.log(
                      `Tempo de respawn excede o limite máximo. Ajustando de ${respawn.time} para ${MAX_RESPAWN_TIME} minutos`,
                    )
                    respawn.time = MAX_RESPAWN_TIME
                  }
                }
              } catch (error) {
            ...
            // Não remover o respawn, apenas limpar o current
            respawn.current = null
            respawn.waitingForAccept = false
            console.log(`Respawn ${respawnKey}: Fila vazia, mantendo respawn vazio`)
          }
          mudancas = true
        } else {
          mudancas = true
        ...
        // Verificar se endTime existe antes de processar
        if (!respawn.endTime) {
          console.log(`Respawn ${respawnKey}: Não tem endTime definido, pulando...`)
          continue
        }
        ...
        if (now >= endTime) {
          console.log(
            `Respawn ${respawnKey}: Tempo esgotado para ${respawn.current.clientNickname} (${respawn.current.clientUniqueIdentifier})`,
          )

          try {
            // Verificar se o cliente ainda está online e se o CLID está atualizado
            let clientAtual = null
            let clientEncontrado = false

            // Primeiro verificar se o uniqueIdentifier existe
            if (respawn.current.clientUniqueIdentifier) {
              // Obter lista de clientes conectados
              const allClients = await ts3.clientList()

              // Procurar o cliente pelo uniqueIdentifier
              const matchingClient = allClients.find(
                (c) => c.uniqueIdentifier === respawn.current.clientUniqueIdentifier,
              )

              if (matchingClient) {
                clientAtual = matchingClient
                clientEncontrado = true

                // Atualizar o CLID se necessário
                if (respawn.current.clid !== matchingClient.clid) {
                  console.log(
                    `Atualizando CLID do cliente ${respawn.current.clientNickname}: ${respawn.current.clid} -> ${matchingClient.clid}`,
                  )
                  respawn.current.clid = matchingClient.clid
                }
              }
            }

            // Só enviar mensagem se o cliente foi encontrado online
            if (clientEncontrado) {
              console.log(
                `Enviando notificação de término para ${respawn.current.clientNickname} (CLID: ${respawn.current.clid})`,
              )
              ...
              // Envia poke
              await ts3.clientPoke(respawn.current.clid, "Seu tempo de respawn terminou.")
            } else {
              console.log(`Cliente ${respawn.current.clientNickname} não está online, não enviando notificação`)
            }
          } catch (error) {
          ...
          if (respawn.queue && respawn.queue.length > 0) {
            ...
            // Verificar se o próximo cliente é válido
            if (!nextClient || !nextClient.clientUniqueIdentifier) {
              console.error(`Próximo cliente inválido na fila do respawn ${respawnKey}`)
              // Remover cliente inválido e continuar com o próximo da fila
              continue
            }
...
async function handleAddExtraTimeCommand(client, groupId, extraTime) {
  try {
    ...
    // Adicionar o tempo extra ao grupo
    extraRespawnTimes[groupIdNumber] = extraTimeMinutes
    
    // Salvar os tempos extras no arquivo
    saveExtraRespawnTimes();
    
    // Informar ao usuário sobre o tempo extra adicionado e o limite máximo
    const MAX_RESPAWN_TIME = 210 // 3 horas e 30 minutos
    await ts3.sendTextMessage(
      client.clid,
      1,
      `Tempo extra de ${extraTimeMinutes} minutos adicionado ao grupo ${groupIdNumber}. Lembre-se que o tempo máximo de respawn é de ${MAX_RESPAWN_TIME} minutos (3h30).`,
    )
  } catch (error) {
...
// No início do seu arquivo, junto com as outras constantes de arquivo
const EXTRA_TIMES_FILE = './extra_respawn_times.json';

// NÃO redeclare extraRespawnTimes, use a que já existe
// const extraRespawnTimes = {}; // Esta linha já existe no seu código

// Função para carregar tempos extras do arquivo
function loadExtraRespawnTimes() {
  try {
    if (fsr.existsSync(EXTRA_TIMES_FILE)) {
      const loadedTimes = JSON.parse(fsr.readFileSync(EXTRA_TIMES_FILE, 'utf8'));
      
      // Limpar o objeto existente e copiar os valores carregados
      Object.keys(extraRespawnTimes).forEach(key => delete extraRespawnTimes[key]);
      Object.assign(extraRespawnTimes, loadedTimes);
      
      console.log('Tempos extras carregados:', Object.keys(extraRespawnTimes).length);
    } else {
      // Cria o arquivo se não existir com os valores atuais
      saveExtraRespawnTimes();
      console.log('Arquivo de tempos extras criado com valores padrão.');
    }
  } catch (error) {
    console.error('Erro ao carregar tempos extras:', error);
    // Garante que o arquivo seja criado mesmo em caso de erro
    saveExtraRespawnTimes();
  }
}

// Função para salvar tempos extras no arquivo
function saveExtraRespawnTimes() {
  try {
    fsr.writeFileSync(EXTRA_TIMES_FILE, JSON.stringify(extraRespawnTimes, null, 2), 'utf8');
    console.log('Tempos extras salvos com sucesso.');
  } catch (error) {
    console.error('Erro ao salvar tempos extras:', error);
  }
}

// Carregar tempos extras ao iniciar
loadExtraRespawnTimes();


async function handleRemoveExtraTimeCommand(client, groupId) {
  try {
    const clientInfo = await ts3.getClientById(client.clid)
    ...
    if (!(await isMasterAdm(clientInfo))) {
      await ts3.sendTextMessage(client.clid, 1, "Você não tem permissão para usar o comando !removeextratime.")
      return
    }
    ...
    // Verificar se o grupo tem tempo extra configurado
    if (!extraRespawnTimes[groupIdNumber]) {
      await ts3.sendTextMessage(client.clid, 1, `O grupo ${groupIdNumber} não tem tempo extra configurado.`)
      return
    }

    // Remover o tempo extra do grupo
    delete extraRespawnTimes[groupIdNumber]
    ...
    await ts3.sendTextMessage(
      client.clid,
      1,
      `Tempo extra removido do grupo ${groupIdNumber}.`
    )
  } catch (error) {
    console.error("Erro ao processar comando !removeextratime:", error)
    try {
      await ts3.sendTextMessage(client.clid, 1, "Ocorreu um erro ao processar seu comando. Por favor, tente novamente.")
    } catch (error) {
      console.error("Erro ao enviar mensagem de erro:", error)
...
async function handleListExtraTimeCommand(client) {
  try {
    const clientInfo = await ts3.getClientById(client.clid)
    ...
    if (!(await isMasterAdm(clientInfo))) {
      await ts3.sendTextMessage(client.clid, 1, "Você não tem permissão para usar o comando !listextratime.")
      return
    }

    // Verificar se há tempos extras configurados
    const extraTimesCount = Object.keys(extraRespawnTimes).length
    if (extraTimesCount === 0) {
      await ts3.sendTextMessage(client.clid, 1, "Não há tempos extras configurados.")
      return
    }

    // Construir a mensagem com a lista de tempos extras
    let message = "[b]Tempos extras configurados:[/b]\n\n"
    
    for (const [groupId, extraTime] of Object.entries(extraRespawnTimes)) {
      // Tentar obter o nome do grupo
      let groupName = "Desconhecido"
      try {
        const groups = await ts3.serverGroupList()
        const group = groups.find(g => g.sgid === groupId)
        if (group) {
      ...
      message += `Grupo ${groupId} (${groupName}): +${extraTime} minutos\n`
    }
    
    await ts3.sendTextMessage(client.clid, 1, message)
  } catch (error) {
    console.error("Erro ao processar comando !listextratime:", error)
    try {
      await ts3.sendTextMessage(client.clid, 1, "Ocorreu um erro ao processar seu comando. Por favor, tente novamente.")
    } catch (error) {
      console.error("Erro ao enviar mensagem de erro:", error)
...
async function getRespawnTime(clientInfo) {
  ...
  // Aplicar o limite máximo de 3:30 horas (210 minutos)
  const MAX_RESPAWN_TIME = 210 // 3 horas e 30 minutos

  if (totalTime > MAX_RESPAWN_TIME) {
    console.log(`Tempo de respawn excede o limite máximo. Ajustando de ${totalTime} para ${MAX_RESPAWN_TIME} minutos`)
    totalTime = MAX_RESPAWN_TIME
  }

  console.log("Tempo final de respawn:", totalTime) // Log para ver o tempo final
  return totalTime // Retorna o tempo total de respawn limitado ao máximo
}
...
async function handleRespCommand(client, respawnNumber, customTime) {
  try {
    ...
    const isExempt =
      ...
      clientServerGroups.includes(editor.toString())
    ...
    if (respawnTime > MAX_RESPAWN_TIME) {
      console.log(`Tempo de respawn excede o limite máximo. Ajustando de ${respawnTime} para ${MAX_RESPAWN_TIME} minutos`)
      respawnTime = MAX_RESPAWN_TIME
    }

    // Criar objeto de dados do cliente com uniqueIdentifier e tempo personalizado
    const clientData = {
      clid: client.clid,
      clientNickname: client.nickname,
      clientUniqueIdentifier: clientInfo.uniqueIdentifier,
      customTime: customTime ? respawnTime : null, // Armazenar o tempo personalizado
    }
    ...
    if (!filaRespawns[respawnKey]) {
      ...
      // Informar o tempo personalizado na mensagem se aplicável
      let mensagem = `Você entrou na fila do respawn ${respawnName} (${respawnNumber}). Aguarde sua vez.`
      if (customTime) {
        const hours = Math.floor(respawnTime / 60)
        const minutes = respawnTime % 60
        const timeDisplay =
          hours > 0
            ? `${hours} hora${hours > 1 ? "s" : ""} e ${minutes} minuto${minutes !== 1 ? "s" : ""}`
            : `${minutes} minuto${minutes !== 1 ? "s" : ""}`
        mensagem += ` Seu tempo será de ${timeDisplay} quando chegar sua vez.`
      }
      
      await ts3.sendTextMessage(client.clid, 1, mensagem)
    }
...
async function isMasterAdm(clientInfo) {
  ...
  return (
    ...
    clientServerGroups.includes(planilhado.toString())
  )
}
...
async function handleRespPauseCommand(client, respawnNumber) {
  try {
    ...
    // Se estiver pausando (respawn não está pausado atualmente)
    if (!respawn.pausado) {
      // Marcar como pausado
      respawn.pausado = true
      
      // Salvar a data atual como data de pausa
      respawn.pauseDate = new Date().toISOString()
      
      if (respawn.current && respawn.endTime) {
        const agora = new Date()
        const endTime = new Date(respawn.endTime)
        const tempoRestanteMs = endTime.getTime() - agora.getTime()

        // Salvar o tempo restante em milissegundos
        respawn.tempoRestantePausa = tempoRestanteMs > 0 ? tempoRestanteMs : 0
        
        // Salvar o tempo decorrido até o momento da pausa
        const startTime = new Date(respawn.startTime)
        const tempoDecorridoMs = agora.getTime() - startTime.getTime()
        respawn.tempoDecorridoPausa = tempoDecorridoMs > 0 ? tempoDecorridoMs : 0

        // Guardar o endTime original e remover o endTime atual
        respawn.endTimeOriginal = respawn.endTime
        respawn.endTime = null

        console.log(
          `Respawn ${respawnNumber} PAUSADO. Tempo restante salvo: ${Math.ceil(respawn.tempoRestantePausa / (1000 * 60))} minutos`
        )
      }
      
      await ts3.sendTextMessage(client.clid, 1, `O respawn ${respawnKey} foi pausado.`)
    }
    // Se estiver despausando (respawn está pausado atualmente)
    else {
      // Marcar como não pausado
      respawn.pausado = false
      
      if (respawn.current && respawn.tempoRestantePausa !== null && respawn.tempoRestantePausa !== undefined) {
        const agora = new Date()
        
        // IMPORTANTE: Ajustar o startTime para que o tempo decorrido seja o mesmo de quando foi pausado
        // Isso faz com que o cronômetro "volte" para o tempo que estava quando foi pausado
        if (respawn.tempoDecorridoPausa !== null && respawn.tempoDecorridoPausa !== undefined) {
          const novoStartTime = new Date(agora.getTime() - respawn.tempoDecorridoPausa)
          respawn.startTime = novoStartTime.toISOString()
        }
        
        // Calcular o novo endTime com base no tempo restante
        const novoEndTime = new Date(agora.getTime() + respawn.tempoRestantePausa)
        respawn.endTime = novoEndTime.toISOString()

        // Calcular o tempo restante em minutos para exibição
        const tempoRestanteMinutos = Math.ceil(respawn.tempoRestantePausa / 60000)
        
        console.log(`Respawn ${respawnNumber} DESPAUSADO. Tempo restante: ${tempoRestanteMinutos} minutos. Novo tempo final: ${novoEndTime.toLocaleString()}`)
        
        // Limpar as variáveis temporárias
        respawn.tempoRestantePausa = null
        respawn.tempoDecorridoPausa = null
        respawn.pauseDate = null
        respawn.endTimeOriginal = null
      }
...
async function handleSetRespawnTimeCommand(client, groupId, time) {
  try {
    ...
    // Verificar se o tempo excede o limite máximo
    const MAX_RESPAWN_TIME = 210 // 3 horas e 30 minutos
    let finalTime = timeInMinutes;
    
    if (timeInMinutes > MAX_RESPAWN_TIME) {
      finalTime = MAX_RESPAWN_TIME;
      await ts3.sendTextMessage(
        client.clid,
        1,
        `O tempo solicitado (${timeInMinutes} minutos) excede o limite máximo de ${MAX_RESPAWN_TIME} minutos (3h30). O tempo foi ajustado para o limite máximo.`
      )
    }

    // Atualiza o tempo de respawn no objeto
    customRespawnTimes[groupIdNumber] = finalTime
    ...
    await ts3.sendTextMessage(
      client.clid,
      1,
      `Tempo de respawn para o grupo ${groupIdNumber} definido como ${finalTime} minutos.`,
    )
  } catch (error) {
...
async function handleRespInfoCommand(client, respawnNumber) {
  try {
    ...
    if (filaRespawns[respawnKey].current) {
      ...
      // Mostrar tempo personalizado do cliente atual se existir
      if (filaRespawns[respawnKey].current.customTime !== null && 
          filaRespawns[respawnKey].current.customTime !== undefined) {
        const customTime = filaRespawns[respawnKey].current.customTime
        const hours = Math.floor(customTime / 60)
        const minutes = customTime % 60
        infoMessage += ` (Tempo total: ${hours}h ${minutes.toString().padStart(2, '0')}m)`
      }
      ...
      if (filaRespawns[respawnKey].waitingForAccept) {
        ...
        if (pausedRespawns[respawnKey]) {
          ...
          infoMessage += `Tempo restante: ${hours}h ${minutes.toString().padStart(2, '0')}m\n`
        }
      }
    ...
    if (filaRespawns[respawnKey].queue.length > 0) {
      ...
      filaRespawns[respawnKey].queue.forEach((user, index) => {
        let userInfo = `  ${index + 1}. ${user.clientNickname}`
        
        // Adicionar informação de tempo personalizado se existir
        if (user.customTime !== null && user.customTime !== undefined) {
          const hours = Math.floor(user.customTime / 60)
          const minutes = user.customTime % 60
          userInfo += ` - Tempo: ${hours}h ${minutes.toString().padStart(2, '0')}m`
        }
...
async function handleRespStopCommand(client) {
  try {
    ...
    if (!(await isMasterAdm(clientInfo))) {
      await ts3.sendTextMessage(client.clid, 1, "Você não tem permissão para usar o comando.")
      return
    }
    ...
    await ts3.sendTextMessage(client.clid, 1, "O sistema de respawn foi pausado. Use !respunpauseall para retomar.")
  } catch (error) {
  ...
  try {
    ...
    for (const respawnKey in filaRespawns) {
      ...
      if (respawn.current && respawn.current.clientUniqueIdentifier === uniqueId && respawn.waitingForAccept) {
        ...
        // IMPORTANTE: Verificar se o cliente tem um tempo personalizado
        // Se tiver, garantir que seja usado
        if (respawn.current.customTime !== null && respawn.current.customTime !== undefined) {
          console.log(
            `Cliente ${respawn.current.clientNickname} tem tempo personalizado: ${respawn.current.customTime} minutos`,
          )
          respawn.time = respawn.current.customTime
        }
        ...
        console.log(
          `Respawn ${respawnKey} aceito por ${respawn.current.clientNickname} com tempo de ${respawn.time} minutos`,
        )
...
async function cleanupDisconnectedClients() {
  try {
    console.log("Iniciando atualização de CLIDs para clientes reconectados...")
    const filaRespawns = await loadFilaRespawns()
    let mudancas = false
    ...
    for (const respawnKey in filaRespawns) {
      ...
      // Atualizar CLID do cliente atual se ele estiver online
      if (respawn.current && respawn.current.clientUniqueIdentifier) {
        const matchingClient = allClients.find((c) => c.uniqueIdentifier === respawn.current.clientUniqueIdentifier)
        if (matchingClient && matchingClient.clid !== respawn.current.clid) {
          respawn.current.clid = matchingClient.clid
          console.log(`CLID atualizado para cliente atual: ${respawn.current.clientNickname}`)
          mudancas = true
        }
      }

      // Atualizar CLIDs dos clientes na fila
      if (respawn.queue && respawn.queue.length > 0) {
        for (const queuedClient of respawn.queue) {
          if (queuedClient.clientUniqueIdentifier) {
    ...
    if (mudancas) {
      await saveFilaRespawns(filaRespawns)
      await updateRespawnChannel()
      console.log("Atualização de CLIDs concluída com mudanças.")
    } else {
      console.log("Atualização de CLIDs concluída sem mudanças.")
    }
  } catch (error) {
    console.error("Erro durante a atualização de CLIDs:", error)
  }
}


// Arquivo para armazenar o horário de reset
const resetConfigFile = path.join(__dirname, "reset_config.json");

// Variável para armazenar o job agendado
let resetJob = null;

// Função para carregar a configuração de reset
async function loadResetConfig() {
  try {
    const data = await fs.readFile(resetConfigFile, "utf8");
    return JSON.parse(data);
  } catch (error) {
    if (error.code === "ENOENT") {
      // Arquivo não existe, criar com configuração padrão (05:07)
      const defaultConfig = { resetTime: "05:07" };
      await saveResetConfig(defaultConfig);
      console.log("Arquivo reset_config.json criado com horário padrão 05:07.");
      return defaultConfig;
    }
    console.error("Erro ao carregar configuração de reset:", error);
    return { resetTime: "05:07" }; // Valor padrão em caso de erro
  }
}

// Função para salvar a configuração de reset
async function saveResetConfig(config) {
  try {
    await fs.writeFile(resetConfigFile, JSON.stringify(config, null, 2), "utf8");
    console.log(`Configuração de reset salva: ${config.resetTime}`);
  } catch (error) {
    console.error("Erro ao salvar configuração de reset:", error);
  }
}

// Função para limpar a fila de respawns
async function clearRespawnQueue() {
  try {
    console.log("Executando limpeza programada da fila de respawns...");
    
    // Salvar um objeto vazio no arquivo fila.json
    await saveFilaRespawns({});
    
    console.log("Fila de respawns limpa com sucesso pelo agendamento diário.");
    
    // Atualizar o canal de respawn para refletir a fila vazia
    await updateRespawnChannel();
  } catch (error) {
    console.error("Erro ao limpar fila de respawns:", error);
  }
}

// Função para agendar o reset diário
function scheduleReset(timeString) {
  ...
  // Criar regra de agendamento (todos os dias no horário especificado)
  const rule = new schedule.RecurrenceRule();
  rule.hour = hours;
  rule.minute = minutes;
  rule.tz = 'America/Sao_Paulo'; // Definir fuso horário
  
  // Agendar o job
  resetJob = schedule.scheduleJob(rule, clearRespawnQueue);
  
  console.log(`Reset diário agendado para ${timeString} (America/Sao_Paulo)`);
}

// Função para inicializar o sistema de reset
async function initResetSystem() {
  try {
    // Carregar configuração
    const config = await loadResetConfig();
    
    // Agendar reset com o horário configurado
    scheduleReset(config.resetTime);
  } catch (error) {
    console.error("Erro ao inicializar sistema de reset:", error);
  }
}

// Função para lidar com o comando !reset
async function handleResetCommand(client, timeString) {
  try {
    const clientInfo = await ts3.getClientById(client.clid);
    ...
    if (!(await isMasterAdm(clientInfo))) {
      await ts3.sendTextMessage(client.clid, 1, "Você não tem permissão para usar o comando !reset.");
      return;
    }
    ...
    if (!timeRegex.test(timeString)) {
      await ts3.sendTextMessage(client.clid, 1, "Formato de horário inválido. Use o formato HH:MM (ex: 05:07).");
      return;
    }

    // Salvar a nova configuração
    const config = { resetTime: timeString };
    await saveResetConfig(config);
    
    // Reagendar o reset
    scheduleReset(timeString);
    
    await ts3.sendTextMessage(
      client.clid, 
      1, 
      `Horário de reset configurado para ${timeString} (America/Sao_Paulo). A fila de respawns será limpa diariamente neste horário.`
    );
  } catch (error) {
    console.error("Erro ao processar comando !reset:", error);
    try {
      await ts3.sendTextMessage(client.clid, 1, "Ocorreu um erro ao processar seu comando. Por favor, tente novamente.");
    } catch (msgError) {
      console.error("Erro ao enviar mensagem de erro:", msgError);
...
ts3.on("textmessage", (ev) => {
  try {
    ...
    if (message.startsWith("!resp ") || message.startsWith("!respnext ")) {
      ...
      if (args.length >= 2) {
        ...
        ts3.sendTextMessage(ev.invoker.clid, 1, "Uso correto: !resp [número] [tempo opcional] , ou !respnext [número] [tempo opcional]")
      }
    ...
    } else if (message === "!resppauseall") {
      handleRespStopCommand(ev.invoker)
    } else if (message === "!respunpauseall") {
    ...
    } else if (message.startsWith("!resppause ") || message.startsWith("!respunpause ")) {
      const respawnNumber = args[1]
      ...
      // Extrair o horário do comando (ex: !reset 05:07)
      const timeString = args[1];
      handleResetCommand(ev.invoker, timeString);
    }
    else if (message.startsWith("!reset ")) {
      // Extrair o horário do comando (ex: !reset 05:07)
      const timeString = args[1];
      ...
      // Se o comando for usado sem parâmetro, mostrar o horário atual
      (async () => {
        try {
          const config = await loadResetConfig();
          await ts3.sendTextMessage(
            ev.invoker.clid, 
            1, 
            `O horário atual de reset é ${config.resetTime} (America/Sao_Paulo). Use !reset HH:MM para alterar.`
          );
        } catch (error) {
          console.error("Erro ao processar comando !reset sem parâmetros:", error);
        }
      })();
    }
...
async function showHelp(client) {
  try {
      ...
      const isAdminUser = clientInfo.servergroups.includes(editor.toString()) || 
      ...
      [b]!respnext <numero>[/b]
      [i]Adiciona você a fila de um respawn especificado pelo número caso tenha alguém no respawn.[/i]
      ...
      if (isAdminUser) {
          ...
          [b]!respunpause <respawn>[/b]
          [i]Despausa apenas o respawn especifico.[/i]

          [b]!respunpauseall[/b]
          [i]Despausa todos os temporizadores de respawn pausados.[/i]
          ...
          [b]!addgroup <nome do grupo>[/b]
          [i]Adiciona grupo em modo especial onde o bot não remove.[/i]

          [b]!removegroup <nome do grupo>[/b]
          [i]Remove grupo em modo especial onde o bot não remove.[/i]

          [b]!viewgroup[/b]
          [i]Lista todos os grupos no modo especial.[/i]
          `;
      }
...
async function isAdmin(ts3, clid) {
    try {
        ...
        return [serveradminGroupID, editor, botadm, mmove].some(group => clientServerGroups.includes(group.toString()));
    } catch (error) {
...
async function isEditor(ts3, clid) {
    try {
        ...
        // Verifica se o cliente pertence ao grupo admin
        return [editor, botadm].some(group => clientServerGroups.includes(group.toString()));
    } catch (error) {
        console.error("Erro ao verificar se o cliente é admin:", error);
        return false;
    }
...
// Função para carregar o nome do mundo do arquivo set_world.json
async function loadWorldName() {
    try {
        const data = await fs.readFile(worldFilePath, 'utf8');
        const json = JSON.parse(data);
        return json.world || "Firmera"; // Retorna o mundo definido ou "Firmera" como padrão
    } catch (error) {
        console.error('Erro ao carregar o nome do mundo:', error);
        return "Firmera"; // Retorna "Firmera" como padrão em caso de erro
    }
}
...
const enemyPlayerTimes = {}; // Objeto para armazenar os tempos de entrada dos jogadores inimigos

// Função para formatar o tempo online (em minutos) para jogadores inimigos
function formatEnemyPlayerTime(playerName) {
    const timestamp = enemyPlayerTimes[playerName];
    if (!timestamp) {
        return 'Desconhecido'; // Se o jogador não tiver tempo registrado, retorna "Desconhecido"
    }
...
// Função para enviar masspoke informando que o jogador inimigo ficou online
async function sendMasspokeForEnemyOnline(playerName) {
    try {
        const clients = await ts3.clientList({ clientType: 0 }); // Obtém todos os clientes online (não bots)
        ...
        console.log(`Masspoke enviado: ${message}`);
    } catch (error) {
        console.error("Erro ao enviar masspoke:", error);
    }
}
...
async function updateEnemyPlayerChannelDescription(channelId) {
  ...
  // Carregar o nome do mundo do arquivo
  const worldName = await loadWorldName();
  console.log(`Buscando jogadores inimigos no mundo: ${worldName}`);

  let channelDescription = "Jogadores inimigos online:\n\n";
  const currentOnlinePlayers = new Set();
  const onlinePlayerNames = new Set(); // Para rastrear quais jogadores estão online
  ...
  try {
      ...
      if (data.world && data.world.online_players) {
          ...
          for (const player of onlinePlayers) {
              const playerNameLower = player.name.toLowerCase(); // Converte o nome para minúsculas
              onlinePlayerNames.add(playerNameLower); // Adiciona à lista de jogadores online

              if (enemyPlayers.has(playerNameLower)) { // Verifica se o jogador está na lista de inimigos
                  channelDescription += `☠️[b][color=#228B22] ${player.name}[/b] [b] (Level ${player.level}, ${player.vocation})[/b] (Online por: ${formatEnemyPlayerTime(player.name)})\n`;
                  currentOnlinePlayers.add(player.name);
              
                  // Registra o tempo do jogador online (caso ainda não tenha sido registrado)
                  if (!enemyPlayerTimes[player.name]) {
                      enemyPlayerTimes[player.name] = Date.now(); // Registra o tempo de entrada do jogador
              
                      // Enviar masspoke informando que o jogador ficou online
                      await sendMasspokeForEnemyOnline(player.name);
                  }
              }                
          }
      }
  } catch (error) {
      console.error(`Erro ao buscar informações do mundo ${worldName}:`, error);
  }
  ...
  // Adicionar seção para jogadores offline
  channelDescription += "\n\nJogadores inimigos offline:\n\n";
  let offlineCount = 0;

  // Verificar quais jogadores da lista estão offline
  for (const playerName of enemyPlayers) {
      if (!onlinePlayerNames.has(playerName)) {
          // Formatar o nome do jogador (primeira letra maiúscula)
          const formattedName = playerName.charAt(0).toUpperCase() + playerName.slice(1);
          channelDescription += `☠️[b][color=#FF0000] ${formattedName}[/b]\n`;
          offlineCount++;
      }
  }

  if (offlineCount === 0) {
      channelDescription += "Todos os jogadores inimigos estão online.";
  }
  ...
  try {
      const onlineCount = currentOnlinePlayers.size;
      const totalCount = enemyPlayers.size;
      const channelName = `[cspaceri7]✖ Lista de Hunteds (${onlineCount}/${totalCount})✖`;
      await ts3.channelEdit(channelId, { channel_name: channelName });
      console.log(`Nome do canal atualizado para: ${channelName}`);
  } catch (error) {
      console.error("Erro ao atualizar o nome do canal:", error);
...
// Iniciar a atualização automática do canal de ID 35 a cada 60 segundos
...
// Define o caminho para o arquivo JSON
const respawnsFilePath = path.join(__dirname, "respawns.json");
const CHANNEL_ID = 17; // ID do canal onde a lista será exibida

// Função para carregar os respawns do arquivo JSON
async function loadRespawns() {
  try {
    const data = await fs.readFile(respawnsFilePath, "utf8");
    return JSON.parse(data);
  } catch (error) {
...
// Função para salvar os respawns no arquivo JSON
async function saveRespawns(respawns) {
  try {
    await fs.writeFile(respawnsFilePath, JSON.stringify(respawns, null, 2));
    console.log("Respawns salvos com sucesso.");
    return true;
  } catch (error) {
    console.error("Erro ao salvar os respawns:", error);
    return false;
  }
}

// Função para adicionar um novo respawn
async function addRespawn(location, id, description, user) {
  try {
    // Carrega os respawns atuais
    const respawns = await loadRespawns();

    // Verifica se a localização existe, se não, cria
    if (!respawns[location]) {
      respawns[location] = {};
    }

    // Adiciona o novo respawn
    respawns[location][id] = description;

    // Salva os respawns atualizados
    const saved = await saveRespawns(respawns);

    if (saved) {
      // Atualiza a descrição do canal
      await updateRespawnChannelDescription(CHANNEL_ID);

      // Envia mensagem de confirmação para o usuário
      await ts3.sendTextMessage(user.clid, 1, `Respawn '${id}' adicionado com sucesso em ${location}!`);
      return true;
    }
    return false;
  } catch (error) {
    console.error("Erro ao adicionar respawn:", error);
    return false;
  }
}

// Função para remover um respawn específico
async function removeRespawn(location, id, user) {
  try {
    // Carrega os respawns atuais
    const respawns = await loadRespawns();
    ...
    if (!respawns[location]) {
      await ts3.sendTextMessage(user.clid, 1, `Erro: Localização '${location}' não encontrada.`);
      return false;
    }

    // Verifica se o respawn existe
    if (!respawns[location][id]) {
      await ts3.sendTextMessage(user.clid, 1, `Erro: Respawn '${id}' não encontrado em ${location}.`);
      return false;
    }

    // Remove o respawn
    delete respawns[location][id];

    // Se a localização ficou vazia, remove-a também
    if (Object.keys(respawns[location]).length === 0) {
      delete respawns[location];
    }
    ...
    if (saved) {
      ...
      // Envia mensagem de confirmação para o usuário
      await ts3.sendTextMessage(user.clid, 1, `Respawn '${id}' removido com sucesso de ${location}!`);
      return true;
    }
    return false;
  } catch (error) {
    console.error("Erro ao remover respawn:", error);
    return false;
  }
}

// Função para gerar a descrição do canal com a lista de respawns formatada
async function generateRespawnDescription() {
  try {
    const respawns = await loadRespawns();

    // Inicia a descrição com o título
    let description = "[COLOR=#008000][size=+3]Respawn Number List[/size][/COLOR]\n[url=https://luzibra.jowbot.com.br/resplist.html][size=+2][b]CLIQUE AQUI PARA VER TODOS[/b][/size][/url]\n\n";

    // Adiciona cada localização e seus respawns
    for (const location of Object.keys(respawns).sort()) {
      description += `\n\n[size=+1][b]${location}[/b][/size]\n`;
      description += "[table]\n";

      // Adiciona cada respawn da localização
      const locationRespawns = respawns[location];
      for (const id of Object.keys(locationRespawns).sort()) {
        description += `[tr][td]${id}[/td][td]${locationRespawns[id]}[/td][/tr]\n\n`;
      }
    ...
    console.error("Erro ao gerar descrição dos respawns:", error);
    return "Erro ao gerar lista de respawns.";
  }
}

// Função para atualizar a descrição do canal
async function updateRespawnChannelDescription(channelId) {
  try {
    const description = await generateRespawnDescription();

    // Atualiza a descrição do canal
    await ts3.channelEdit(channelId, { channel_description: description });
    console.log("Descrição do canal de respawns atualizada com sucesso!");
    return true;
  } catch (error) {
    console.error("Erro ao atualizar a descrição do canal de respawns:", error);
    return false;
  }
}

// Função para iniciar a atualização automática do canal
function startRespawnAutoUpdate(channelId, intervalMs = 300000) { // 5 minutos por padrão
  updateRespawnChannelDescription(channelId); // Primeira execução imediata
  setInterval(() => {
    updateRespawnChannelDescription(channelId);
  }, intervalMs);
}

// Carregar os respawns quando o bot iniciar
loadRespawns();

// Iniciar a atualização automática do canal
startRespawnAutoUpdate(CHANNEL_ID);

// Manipulador de eventos para os comandos
ts3.on("textmessage", async (ev) => {
  const message = ev.msg;

  if (message.startsWith("!addresp")) {
    // Formato esperado: !addresp Localização ID Descrição
    const parts = message.split(" ");

    if (parts.length < 4) {
      await ts3.sendTextMessage(ev.invoker.clid, 1, "Uso correto: !addresp Localização ID Descrição");
      return;
    }
    ...
    await addRespawn(location, id, description, ev.invoker);
  } 
  else if (message.startsWith("!removeresp")) {
    // Formato esperado: !removeresp Localização ID
    const parts = message.split(" ");

    if (parts.length < 3) {
      await ts3.sendTextMessage(ev.invoker.clid, 1, "Uso correto: !removeresp Localização ID");
      return;
    }
    ...
    await removeRespawn(location, id, ev.invoker);
  }
});
