...
window.loadPage = async function(pageName) {
    ...
    if (pageName === 'bosses') {
        // Toda a lógica de bosses foi removida conforme solicitado.
    }
};
...
function updateRespawnTable(fila, allRespawnNames) {
    ...
    respawnTableBody.innerHTML = rowsData.map(entry => {
        ...
        const renderPlayerName = (user) => {
            ...
            // LÓGICA ATUALIZADA PARA RENDERIZAR MAKER OU JOGADOR NORMAL
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.appSocket.on('bot:private_message', ({ sender, message }) => {
        addLogMessage('Bot', message, 'bot'); // Re-adicionei o sender para consistência
    ...
    if (saveRespawnOptionsBtn) {
        saveRespawnOptionsBtn.addEventListener('click', () => {
            ...
            // Envia ambas as listas atualizadas para o servidor
...
function renderAllUsersTab() {
        ...
        filteredUsers
            ...
            .forEach(([email, account]) => {
                ...
                const charsHtml = (account.tibiaCharacters && account.tibiaCharacters.length > 0) 
                ? account.tibiaCharacters.map(char => `<div class="char-details">- <strong>${char.characterName}</strong> (Lvl: ${char.level || 'N/A'}, Voc: ${char.vocation || 'N/A'}, Rank: ${char.guildRank || 'N/A'})</div>`).join('') : '<div class="char-details">- Nenhum personagem registrado.</div>';
    ...
    function renderUserList() {
        ...
        Object.entries(allUsers)
            ...
            .forEach(([email, account]) => {
                ...
                item.innerHTML = `<div class="user-info"><span>${account.name} (${mainChar.characterName})</span><span class="user-rank">${mainChar.guildRank ||
 'Sem Rank'}</span></div><div class="user-groups-pills">${userGroups.map(gid => { const group = allGroups.find(g => g.id === gid); return `<span class="group-pill">${group ? group.name : '??'}</span>`; }).join('')}</div>`;
