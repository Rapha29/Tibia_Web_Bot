...
io.on('connection', (socket) => {
    ...
    socket.on('points:addMountainPiece', async ({ players }) => {
        const user = webUsers.get(socket.id);
        if (user && user.character && adminRanks.includes(user.character.guildRank?.toLowerCase())) {
            await pointsLogic.addPoints('Mountain Piece', 2, players, 'Participação');
            socket.emit('bot:response', { type: 'success', text: 'Pontos de Mountain Piece adicionados.'});
            const data = await pointsLogic.getPointsData();
            io.emit('points:dataUpdated', data);
        }
    });
