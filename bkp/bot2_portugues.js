...
ts3.on("clientconnect", async (event) => {
    try {
        const welcomeMessages = [
            `[color=blue][b]Servidor Hospedado e Configurado por Tobot[/b] Atualizações e Inovações diárias![/color]`,
            `[color=green][b]Estamos na fase *BETA*, e contamos com a sua ajuda para melhorar o serviço! Atualizações e Inovações diárias![/color]`,
...

// Armazena os respawns ativos e a fila
let activeRespawns = {};
let respawnData = {}; // Para armazenar os dados do respawn.json

// Tempos de respawn personalizados por grupo
const customRespawnTimes = {
  96: 210, // 3:30 horas em minutos 
  70: 180, // 3 horas em minutos
  30: 120, // 2:30 horas em minutos
  102: 90   // 1 hora em minutos
};
...
async function loadRespawnData() {
    ...
    try {
        const data = await fs.readFile(filePath, 'utf8');
        respawnData = JSON.parse(data);
        
    } catch (error) {
        console.error("Erro ao carregar os dados de respawn:", error);
        respawnData = {}; // Inicializa como vazio se o arquivo não existir
    }
}
...
async function formatClientName(nickname, uniqueIdentifier, clid) {
    // Codificar caracteres especiais no nickname para a URL e escapar colchetes e barras invertidas
    const encodedNickname = nickname
        .replace(/\\/g, '%5C') // Escapar barras invertidas
        .replace(/\[/g, '%5C%5B') // Escapar colchetes [
        .replace(/\]/g, '%5C%5D') // Escapar colchetes ]
        .replace(/ /g, '%20'); // Codificar espaços

    // Retornar a URL formatada corretamente com o nickname original para exibição
...
async function updateRespawnChannel() {
    ...
    for (const respawnNumber in activeRespawns) {
        ...
        // Garantir que as informações do cliente estão atualizadas
        ...
        if (respawn.waitingForAccept) {
            description += `[td]Aguardando !aceitar (${respawn.acceptanceTime}m)[/td]`;
        } else {
            description += `[td]${Math.floor(respawn.time / 60)}h ${respawn.time % 60}m[/td]`;
        }

        if (respawn.queue.length > 0) {
            const nextClient = respawn.queue[0];
            // Garantir que as informações do próximo cliente estão atualizadas
        ...
        if (!isRespawnPaused) {
            if (respawn.waitingForAccept) {
                ...
                if (respawn.acceptanceTime <= 0) {
                    // Remove the current player if they didn't accept
...
async function hasGuildBankPermission(clientInfo) {
    ...
    // Se o usuário estiver no grupo respblockGroupID, ele não pode usar o comando
...
async function getRespawnTime(clientInfo) {
    ...
    for (const groupId in customRespawnTimes) {
        if (clientServerGroups.includes(groupId)) {
            baseTime = customRespawnTimes[groupId];
            break;
        }
    }
...
async function handleRespCommand(client, respawnNumber) {
    ...
    // Verifica o limite de respawns por cliente
    ...
    // Verifica se o cliente já está no respawn específico
    ...
    // Conta quantos respawns ativos o cliente tem
    ...
    const respawnTime = await getRespawnTime(clientInfo);
    ...
    if (!activeRespawns[respawnNumber]) {
        ...
        await ts3.sendTextMessage(client.clid, 1, `Você está com o respawn de ${respawnName} número ${respawnNumber}. Digite !aceitar para começar seu tempo de ${respawnTime / 60} horas.`);
        
        // Incrementa os contadores de respawn do cliente
        if (!isExempt) {
        ...
        const respawnName = getRespawnName(respawnNumber);
        await ts3.sendTextMessage(client.clid, 1, `Você entrou na fila do respawn ${respawnName} número  ${respawnNumber}. Aguarde sua vez.`);
    }
...
async function handleRespDelCommand(client, respawnNumber) {
    ...
    // Verifica se o cliente pertence ao grupo GuildBank
    if (!await hasGuildBankPermission(clientInfo)) {
    ...
    if (respawn.current && respawn.current.clid === client.clid) {
        if (respawn.queue.length > 0) {
            ...
            // Envia um poke ao próximo jogador da fila
            await ts3.clientPoke(nextClient.clid, "É a sua vez! Digite !aceitar para começar seu tempo de respawn.");

        } else {
            delete activeRespawns[respawnNumber]; // Remove o respawn se não houver mais ninguém na fila
        }

        // Decrementa o contador de respawns ativos do cliente
        if (clientRespawnCount[client.clid]) {
            clientRespawnCount[client.clid].current--;
        }

        await ts3.sendTextMessage(client.clid, 1, `Você saiu do respawn ${respawnNumber}.`);
    } else {
        ...
        if (index !== -1) {
            ...
            await ts3.sendTextMessage(client.clid, 1, "ocê não está neste respawn ou na fila.");
        }
    }
...
async function handleRespKickCommand(client, respawnNumber) {
    ...
    // Verifica se o cliente pertence ao grupo de admin
    if (!await isMasterAdm(clientInfo)) {
    ...
    if (respawn.current) {
        ...
        // Decrementa o contador de respawns ativos do cliente kickado
        ...
        if (respawn.queue.length > 0) {
            ...
            delete activeRespawns[respawnNumber]; // Libera o respawn se a fila estiver vazia
        }
...
async function handleSetRespawnTimeCommand(client, groupId, time) {
    ...
    await ts3.sendTextMessage(client.clid, 1, `Tempo de respawn para o grupo ${groupIdNumber} definido para ${timeInMinutes} minutos.`);
}
...
async function handleRespInfoCommand(client, respawnNumber) {
    ...
    if (respawn.current) {
        ...
        if (respawn.waitingForAccept) {
            ...
            infoMessage += `Tempo restante: ${Math.floor(respawn.time / 60)}h ${respawn.time % 60}m\n`;
        }
        infoMessage += '\n';
    } else {
...
async function handleAceitarCommand(client) {
    for (const respawnNumber in activeRespawns) {
        ...
        if (respawn.current && respawn.current.clid === client.clid && respawn.waitingForAccept) {
            ...
            await ts3.sendTextMessage(client.clid, 1, `Você aceitou o respawn ${respawnNumber}. Seu tempo de ${respawn.time / 60} horas começou.`);
            await updateRespawnChannel();
            return;
        }
    }
...
// Carregar a fila de respawns e os dados de respawn ao iniciar
