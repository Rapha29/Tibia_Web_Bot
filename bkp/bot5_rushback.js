...
const guildAliada = setGuild && setGuild.guild ? setGuild.guild.replace(/ /g, '%20') : 'Cantabra Encore';

const guildEnemyData = readJsonFile('./guild_enemy.json');
const guildEnemies = guildEnemyData && guildEnemyData.guilds
...
async function fetchDeaths(guildName) {
    ...
    const allDeaths = await Promise.all(
        onlineMembers.map(async (member) => {
            ...
            const characterData = await fetchWithRetry(`https://api.tibiadata.com/v4/character/${encodeURIComponent(member.name)}`);
            return {
    ...
    const flatDeaths = allDeaths.flatMap(member => member.deaths.map(death => ({
    ...
    const todayDeaths = flatDeaths.filter(death => {
        const deathDate = new Date(death.time);
        return deathDate.getDate() === today.getDate() &&
...
function shortenVocation(vocation) {
    switch (vocation) {
        ...
        case "Sorcerer": return "MS";
        case "Druid": return "ED";
        case "Paladin": return "RP";
        case "Knight": return "EK";
        default: return vocation.slice(0, 2); // Abreviação genérica
    }
}
...
async function updateChannelDescription(ts3, channelId, deaths) {
    ...
    let description = `🛑 muertes hoje (${todayDate}):\n\n`;
    description += `[color=red]AMIGO: ${friendDeaths} muertes[/color]\n`;
    description += `[color=green]HUNTED: ${huntedDeaths} muertes[/color]\n\n`;

    if (deaths.length === 0) {
        description += "Nenhuma morte hoje.";
    } else {
        for (const death of deaths) {
            const guildPrefix = death.guild === guildAliada ? '[FRIEND MURIO]' : '[HUNTED MURIO]';
...
async function massPoke(ts3, deaths) {
    ...
    try {
        ...
        console.log('MassPoke concluído com as muertes novas.');
    } catch (error) {
        console.error("Erro ao fazer massPoke:", error);
    }
}

let lastHuntedOnline = {};
let lastLevels = {}; // Armazena o último nível conhecido de cada jogador
